...
del cantó del microcontrolador com afegint contro\l.ladors al \est{kernel} de
Linux), el dispositiu s'identifica com un mestre \acro{i2c} de cara a
l'ordinador, i apareix a \fitx{/dev/i2c-X} on X és el nombre de dispositiu.
...
\begin{itemize}
    \item Aquest \acro{tfg} pretén ser el màxim de versàtil possible i utilitzar
...
un codi en C que utilitza \est{libusb} per a llegir dades del sensor. Només va
...
es va modificar el programa per a deixar d'utilitzar \est{libusb} i utilitzar
\est{libi2c}, la llibreria de Linux per a comunicar-se amb dispositius
...
\est|micronucleus| per a que, mentre s'estigui executant el \est{bootloader}, es 
...
\acro{avr} i es detallen les peculiaritats i afegits que requereix el sensor que
s'utilitza per al projecte.

\subsection{Protocol I2C en l'arquitectura AVR}

El protocol \acro{i2c} utilitza dues línies bidireccionals, una per les dades i una per
...
la part de \acro{v-usb} i la part de \est{i2c} i del sensor. Tot i això s'ha
decidit anar més enllà, atrevint-se a separar fins i tot les funcions del mòdul
\acro{i2c} de les específiques per a comunicar-se amb el sensor, o el codi per
a calibrar l'osci\l.lador intern del que configura el dispositiu \acro{hid}. 
...
\est{In-Circuit Serial Programming}, és una forma prou comuna per a
