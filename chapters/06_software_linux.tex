...
\est{xrandr} \cite{Pyrandr}. S'ha vist en l'apartat \ref{subsec:xrandr} que la
llibreria
...
\begin{itemize}
    ...
    identificadors de \est{xrandr} per a les pantalles, es pot associar una
    ...
    \item S'ha aplicat el concepte de \est{offset}. Pot ser que algunes persones
...
acceleració per a detectar l'orientació del sensor. A \cite{PedleyTilt} es detalla tota la
...
angular. Es pot consultar un exemple de sensor amb més precisió a \cite{6702711}, però no
...
\est{pandoc} \cite{PandocTutorial},
