...
El connector \acro{usb2} està compost de 4 cables: \acro{5v}, \acro{gnd},
...
diferenciats, com per exemple un convertidor d'\acro{usb} a Jack d'audio. Aquest
...
\est{Human Interface Devices}) són una categoria de dispositius que recullen
...
Aquest terme s'utilitza per a definir el concepte \acro{usb-hid}, una de les
classes \acro{usb} amb que un dispositiu pot identificar-se.
...
dispositiu. Es poden veure el llistat de classes i subclasses \acro{hid} a la
documentació oficial d'\acro{usb-if} \cite{HidDefinition}.

El principal avantatge és que els dispositius
\acro{hid} utilitzen protocols ja establerts per a poder ser utilitzats
sense haver d'insta\l.lar programari nou a l'ordinador (el que es coneix com
a \est{plug\&play}). Així doncs, quan es defineix un tipus concret \acro{hid},
s'ha de complir amb l'estandard de comunicació associat.

Un dels altres avantatges d'utilitzar la classe \acro{hid} és que, al ser una capa per
...
%%%%%%%%%%%%%%%%%%%%%%%TODO
A la taula \ref{tab:hid-rd} es mostra REVISE!!!
...
\begin{table}[h!]
    ...
    \begin{tabular}{l p{0.6\linewidth}}
        ...
        \bottomrule
    \end{tabular}
    \caption{Dades a enviar en l'\est{\acro{HID} Report Descriptor}}
    \label{tab:hid-rd}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%TODO
...
%TODO apartat 3.2.1: feature, input -> d'on surten?
\begin{itemize}
...
\section{Implementacions del protocol \acro{usb} en \acro{avr}}

Un cop es coneix el funcionament del protocol \acro{usb} i es sap quines trames
...
El primer que s'ha cercat és microcontroladors amb arquitectura \acro{avr8} que
tinguessin un perifèric \acro{usb} integrat. S'ha trobat la família
\acro{AtMegaXXuX} (on les X poden ser diferents nombres) de l'empresa
\acro{atmel} \cite{AtMega32u4}. Aquests dispositius, que tenen una \emph{u} en
el seu nom (per a remarcar que disposen del perifèric \acro{usb}), permetrien
...
funciona amb un gran nombre de dispositius \acro{avr}. De ben segur que és una
...
demana \acro{lufa}. Aquest és el principal motiu pel que s'acabarà escollint la
...
implementa la capa més baixa de \acro{usb} a l'arquitectura
...
Al no utilitzar perifèrics específics, \acro{v-usb} no pot ca\l.librar amb
precisió el rellotge i nivells de voltatge per a complir el protocol \acro{usb}
...
\footnote{
    Els submòduls de \est{git}, o \est{Git Submodules}, serveixen per a
...
\acro{unix}, concretament amb Linux, degut que aquest sistema és l'objectiu
...
\est{driver} hi ha molta lògica que no donaria temps a explicar en aquest
document.

\input{misc/linux_iio_stack.tex}

\subsubsection*{Capa 1: \acro{libusb}}
...
A Linux, un dispositiu que es pot controlar per \est{libusb} sol apareixer com
a fitxer sota el format \fitx{/dev/usbrawX}, on X és el número de dispositiu.
...
\subsubsection*{Capa 2: \acro{libhid} i \acro{hidapi}}
...
A Linux, un dispositiu que es pot controlar per \est{libhid} sol apareixer com
a fitxer sota el format \fitx{/dev/hidrawX}, on X és el número de dispositiu.
...
\subsubsection*{Capa 3: \acro{libiio}}
...
que existeix un \est{driver} per a \est{hid\_accelerometer\_3d}, pel que
...
\begin{itemize}
    ...
    El dispositiu creat a \fitx{/dev/iio:deviceX} (on X és el número de
...
\footnote{
    Les regles \acro{udev} no poden tenir salts de línia. Per a facilitar la
...
\subsubsection*{Assignar drivers específics}

Com s'ha vist a l'exemple anterior, és possible crear regles \acro{udev} per a
canviar el \est{driver} o controlador d'un dispositiu. També es pot desvincular
controladors mitjançant l'ordre \ord|udevadm|, que, al no modificar cap fitxer
a \fitx{/etc}, no és persistent.

Així doncs, gràcies a aquesta ordre o la configuració de regles \acro{udev}
...
És important coneixer la diferència entre els dos tipus mencionats, ja que en
...
sota l'interfície de programació de \est{Xlib} \cite{Xlib}.
...
indicar com a dependència \est{www-browser} enlloc de, per exemple, \est{firefox}.

\subsection{Servidor gràfic Xorg}

\est{Xorg} és un servidor gràfic per a gestionar des de configuracions de
pantalla fins a detalls de rendiment per a certes finestres. Els programes
utilitzen la llibreria \est{Xlib} per a interactuar amb ell.
...
es vol crear i l'objectiu final: rotar la pantalla.
...
la opció \ord|-X| de \est{ssh}) que permeten diferents sessions gràfiques
